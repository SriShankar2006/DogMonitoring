...
function shouldSuppressLiteRtRuntimeMessage(args) {
  ...
  return message.includes('powerpreference')
    || message.includes('npu accelerator')
    || message.includes('creating litert environment')
    || message.includes('created litert environment')
    || message.includes('adding options to the existing litert environment')
    || message.includes('failed to load opencl library')
    || message.includes('xnnpack delegate')
    || message.includes('created tensorflow lite xnnpack delegate')
    || message.includes('flatbuffer model initialized')
    || message.includes('registeraccelerator')
    || message.includes('xnnpack cpu accelerator')
    || message.includes('gpuenvironment')
    || message.includes('statically linked gpu accelerator registered')
    || message.includes('requestadapter')
    || message.includes('accelerator could not be loaded')
    || message.includes('gpu registry')
    || message.includes('cpu accelerator')
    || message.includes('environment.cc');
}

var defaultPrint = (...args) => {
  if (!shouldSuppressLiteRtRuntimeMessage(args)) {
    console.log.apply(console, args);
  }
};

var defaultPrintErr = (...args) => {
  if (!shouldSuppressLiteRtRuntimeMessage(args)) {
